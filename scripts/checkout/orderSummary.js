...
// the technique we used 1. update the data 2. regenerate the html is called MVC (Model View Controller) we split our code into 3 parts  1. Model all the code that saves and manages the data like all the code in our data folder is called the model bec it saves and manages the data, 2.View takes the data and displays it on the page like generating the data and then put it on html, 3. Controller runs some code when we interact with the page, event listeners are the controller bec they run some code when we interact with the page
// in MVC we divide our code into these 3 parts and then interact with each other with a loop FIRST we use the MODEL to generate the VIEW then we interact with the VIEW it will run the CONTROLLER then the CONTROLLER will update the MODEL and finally we use the updated MODEL to generate the VIEW
